...
class Role {
  constructor() {
    ...
    this.db.query("SHOW COLUMNS FROM role", (err, cols) => {
      ...
      this._pendingColsCallbacks = [];
      console.log(
        "Role model columns detected: idColumn=%s nameColumn=%s",
        this.idColumn,
        this.nameColumn
      );
    });
  ...
  static getAll(callback) {
    db.query("SELECT id_role, nama_role FROM roles", callback);
  }
    ...
    this.ensureColumns((err) => {
      ...
      this.db.query(sql, (err2, results) => {
        ...
        const roles = results.map((r) => ({
          id_role: r.id_role || r.idRole || r.idrole || r.id,
          role_name: r.role_name || r.nama_role || r.roleName || r.role,
        }));
        callback(null, roles);
      });
    });
